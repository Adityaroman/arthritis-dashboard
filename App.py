...
# --- Part 1: Data Loading and Model Training (Global Scope) ---
def load_and_prepare_data(file_path):
    """Loads data from a CSV, trains a model, and prepares all necessary data structures."""
    logging.info(f"Loading data from {file_path}...")
    try:
        df = pd.read_csv(file_path, parse_dates=['date'])
    except FileNotFoundError:
        logging.error(f"ERROR: The data file '{file_path}' was not found. Please run 'python create_csv.py' first.")
...
# Initialize empty lists and handle case where data loading failed
...
app.layout = html.Div(style={'padding': '20px', 'backgroundColor': '#f4f6f9'}, children=[
    ...
    html.Div(style={'display': 'flex', 'gap': '20px', 'flexWrap': 'wrap'}, children=[
        ...
        # Left Column: Controls and On-Demand Prediction
        ...
        html.Div(style={'flex': '3', 'minWidth': '600px'}, children=[
            html.Div(style={'backgroundColor': 'white', 'padding': '20px', 'borderRadius': '5px', 'boxShadow': '0 2px 4px #ccc'}, children=[
                ...
                dcc.Dropdown(id='filter-location', options=LOCATIONS, value=LOCATIONS[0] if LOCATIONS else None, style={'marginBottom': '20px'}),
...
# Callback for On-Demand Prediction
@app.callback(
    Output('prediction-output', 'children'),
    Input('predict-button', 'n_clicks'),
    [State('predict-location', 'value'),
     State('predict-age', 'value'),
     State('predict-gender', 'value'),
     State('predict-severity', 'value')]
)
...
# Callback for updating the analytical charts
@app.callback(
...
def update_charts(location):
    if processed_data is None or location is None:
        # If data loading failed, return empty charts
        empty_fig = go.Figure().update_layout(title_text="No Data Loaded", title_x=0.5)
        return empty_fig, empty_fig

    filtered_df = processed_data[processed_data['location'] == location]
...
if __name__ == '__main__':
    if model is not None: # Only run the app if the model was trained successfully
        app.run(debug=True, port=8050)
    else:
        print("Could not start the dashboard because data loading failed.")
        print("Please run 'python create_csv.py' to generate the data file and try again.")
